...
usage() { echo "Usage: $0 -m MONTH [-d DAY] [-y YEAR] [-h HOME] -c CITY 
...
HOME  #Three letters representing origin airport, defaults to CPH
...
curl "https://api.flysas.com/offers/flights?displayType=CALENDAR&channel=web&bookingFlow=REVENUE&yth=1&outDate=${YEAR}${MONTH}${DAY}&inDate=${YEAR}${MONTH}${DAY}&from=${HOME}&to=${CITY}&pos=dk" > $file
